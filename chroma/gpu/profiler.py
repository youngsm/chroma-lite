...
# ---- Debug counters: boundary classification/material1 histograms ----

def debug_reset(module=None):
    if module is None:
        from chroma.gpu.tools import get_cu_module, cuda_options
        module = get_cu_module('propagate.cu', options=cuda_options)
    reset = module.get_function('chroma_dbg_reset')
    reset(block=(64,1,1), grid=(1,1))


def debug_fetch(module=None):
    if module is None:
        from chroma.gpu.tools import get_cu_module, cuda_options
        module = get_cu_module('propagate.cu', options=cuda_options)

    meshes_dir_ptr, _ = module.get_global('chroma_dbg_mesh_dir_counts')
    meshes_mat1_ptr, _ = module.get_global('chroma_dbg_mesh_mat1_counts')
    wp_dir_ptr, _ = module.get_global('chroma_dbg_wp_dir_counts')
    wp_mat1_ptr, _ = module.get_global('chroma_dbg_wp_mat1_counts')
    # optional by-direction histograms
    try:
        meshes_mat1_by_dir_ptr, _ = module.get_global('chroma_dbg_mesh_mat1_by_dir')
        wp_mat1_by_dir_ptr, _ = module.get_global('chroma_dbg_wp_mat1_by_dir')
        has_by_dir = True
    except Exception:
        has_by_dir = False
    ...
    out = {}
    arr2 = _np.zeros(2, dtype=_np.uint64)
    arr256 = _np.zeros(256, dtype=_np.uint64)
    arr2x256 = _np.zeros((2,256), dtype=_np.uint64)

    _cuda.memcpy_dtoh(arr2, meshes_dir_ptr)
    out['mesh_dir'] = arr2.copy()
    _cuda.memcpy_dtoh(arr256, meshes_mat1_ptr)
    out['mesh_mat1'] = arr256.copy()

    _cuda.memcpy_dtoh(arr2, wp_dir_ptr)
    out['wp_dir'] = arr2.copy()
    _cuda.memcpy_dtoh(arr256, wp_mat1_ptr)
    out['wp_mat1'] = arr256.copy()

    if has_by_dir:
        _cuda.memcpy_dtoh(arr2x256, meshes_mat1_by_dir_ptr)
        out['mesh_mat1_by_dir'] = arr2x256.copy()
        _cuda.memcpy_dtoh(arr2x256, wp_mat1_by_dir_ptr)
        out['wp_mat1_by_dir'] = arr2x256.copy()

    return out


def debug_report(module=None, labels=None) -> str:
    stats = debug_fetch(module)
    mesh_dir = stats['mesh_dir']
    wp_dir = stats['wp_dir']
    mesh_mat1 = stats['mesh_mat1']
    wp_mat1 = stats['wp_mat1']

    def top_indices(arr, k=5):
        import numpy as _np
        idx = _np.argsort(arr)[::-1][:k]
        return [(int(i), int(arr[i])) for i in idx if arr[i] > 0]

    lines = []
    lines.append(f"mesh dir: outside->inside={int(mesh_dir[0])}, inside->outside={int(mesh_dir[1])}")
    lines.append(f"wp   dir: outside->inside={int(wp_dir[0])}, inside->outside={int(wp_dir[1])}")
    lines.append(f"mesh mat1 top: {top_indices(mesh_mat1)}")
    lines.append(f"wp   mat1 top: {top_indices(wp_mat1)}")
    report = "".join(lines)
    from chroma.log import logger
    logger.info(report)
    return report
